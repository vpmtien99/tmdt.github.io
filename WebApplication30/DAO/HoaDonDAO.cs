    ...
    public class HoaDonDAO
    {
        ...
        public int UpdateSLCTHD(int idCTHD, int sl,int slht)
        {
            ...
            if (pb == 1)
            {
                ...
                var mh = db.SanPhams.Find(cthd.SanPhamId);
            ...
            {
                if (db.SanPhams.Find(db.ChiTietHoaDons.Find(idCTHD).SanPhamId).SoLuong < sltd)
                {
                    return 2;
                }
                else
        ...
        public int UpdateSL(int idCTHD,int sl)
        {
            ...
            else if (db.SanPhams.Find(db.ChiTietHoaDons.Find(idCTHD).SanPhamId).SoLuong < sl)
            {
                return 2;
            }
            else
            {
               ...
               cthd.DonGia = db.SanPhams.Find(cthd.SanPhamId).Gia;
        ...
        public bool UpdateHD(int idHD, int idMH)
        {
            ...
            foreach (var item in db.ChiTietHoaDons.Where(i => i.HDId == idHD).ToList())
            {
                if (item.SanPhamId == idMH)
                {
                    int sl = item.SoLuong+1;
                    if (sl > db.SanPhams.Find(idMH).SoLuong)
                    {
                        return false;
                    }
                    else
                    {
                        var cthd = db.ChiTietHoaDons.Find(item.MaCTHD);
                        cthd.SoLuong=sl;
                        cthd.DonGia = db.SanPhams.Find(idMH).Gia;
                        cthd.ThanhTien = cthd.DonGia * cthd.SoLuong;
                        db.SaveChanges();
                        hd.TongTien = TongTien(idHD);
                        db.SaveChanges();
            ...
            if (db.SanPhams.Find(idMH).SoLuong == 0)
            {
                return false;
            }
            else
            {
                ...
                newCthd.Flag = true;
                newCthd.SanPhamId = idMH;
                newCthd.SanPhams = db.SanPhams.Find(idMH);
                newCthd.HDId = hd.MaHD;
                newCthd.HoaDon = db.HoaDons.Find(hd.MaHD);
                newCthd.SoLuong = 1;
                    newCthd.DonGia = db.SanPhams.Find(idMH).Gia;
        ...
        public bool CreateHD(int idKH,int idMH)
        {
            ...
            else
            {
                ...
                hd.TongTien = db.SanPhams.Find(idMH).Gia;
                db.HoaDons.Add(hd);
                db.SaveChanges();
                ...
                cthd.Flag = true;
                cthd.SanPhamId = idMH;
                cthd.SanPhams = db.SanPhams.Find(idMH);
                cthd.HDId = hd.MaHD;
                    cthd.DonGia = db.SanPhams.Find(idMH).Gia;
