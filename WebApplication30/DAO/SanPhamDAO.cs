...
namespace WebApplication30.DAO
{
    public class SanPhamDAO
    {
        ...
        public List<SanPham> SearchPhanKM(int? mamh, string tenmh, string ss, double? tien, int loai, int nsx, int km, string trangthai)
        {
            ...
            if (mamh != null && mamh!=0)
            {
                ...
                list = list.Where(i => i.MaSP == id).ToList();
                return list;
            }
            if (String.IsNullOrEmpty(tenmh) != true)
            {
                list = list.Where(i => i.TenSP.Contains(tenmh)).ToList();
            }
            if (tien != null & tien!=0)
            {
        ...
        public void BoTatCa(List<SanPham> list)
        {
            if (list != null)
            {
                foreach (var item in list)
                {
                    var mh = db.SanPhams.Find(item.MaSP);
                    mh.KhuyenMaiId = 7;
        ...
        public void GanTatCa(List<SanPham> list,int idKM)
        {
            if (list != null)
            {
                foreach (var item in list)
        ...
        public List<SanPham> GetListChuaKM()
        {
            return db.SanPhams.Where(i => i.Flag == true && i.KhuyenMaiId == 7 && i.KhuyenMai.NgayKT<=DateTime.Today ).ToList();
        }
        public List<SanPham> GetListDaKM(int idKM)
        {
            return db.SanPhams.Where(i => i.Flag == true && i.KhuyenMaiId != 7 && i.KhuyenMaiId == idKM).ToList();
        }
        public List<SanPham> GetList()
        {
            return db.SanPhams.Where(i => i.Flag == true).ToList();
        }
        public void GanAnh(SanPham mh,HttpPostedFileBase anhbia, HttpPostedFileBase anh1, HttpPostedFileBase anh2)
        {
            string path = "";
            if (anhbia != null)
            {
        ...
        public void GanAnhUpdate(SanPham mh, HttpPostedFileBase anhbia, HttpPostedFileBase anh1, HttpPostedFileBase anh2,ref string chiTiet)
        {
            int pb = 0;
            string path = "";
            if (anhbia != null)
        ...
        public SanPham GetMH(int idMH)
        {
            return db.SanPhams.Where(i => i.Flag == true && i.MaSP == idMH).FirstOrDefault();
        }
        public int UpdateDoChoi(int idTK,int idMH,string tenmh, string gia, string sl, HttpPostedFileBase anhbia, HttpPostedFileBase anh1, HttpPostedFileBase anh2, int loai, int nsx, int km, DateTime ngay, string mota)
        {
            ...
            {
                string chiTiet = "Đã sửa sản phẩm có 'MÃ': " + idMH+"; ";
                Log log = new Log();
                ...
                SanPham mh = db.SanPhams.Find(idMH);
                if (mh.TenSP != tenmh)
                {
                    chiTiet = chiTiet + "'TÊN' sản phẩm từ " + mh.TenSP + " sang " + tenmh + "; ";
                    mh.TenSP = tenmh;
                }
        ...
        public List<SanPham> GetListTool()
        {
            return db.SanPhams.Where(i => i.Flag == true && i.Loai.TenLoai == "Dụng Cụ").Take(8).ToList();
        }
        public List<SanPham> GetListToolAll()
        {
            return db.SanPhams.Where(i => i.Flag == true && i.Loai.TenLoai == "Dụng Cụ").ToList();
        }
        public List<SanPham> Search(string tenmh, string gia, int loai, int nsx, int km, string tinhtrang)
        {
            var list = GetList();
            if (String.IsNullOrEmpty(tenmh) != true)
            {
        ...
        public void DeleteDoChoi(int idTK,int idMH)
        {
            ...
            SanPham mh = db.SanPhams.Find(idMH);
            log.ChiTiet = "Đã XÓA Mô Hình có 'MÃ': " + mh.MaSP + "; 'TÊN' mô hình: " + mh.TenSP;
            mh.Flag = false;
            db.Logs.Add(log);
            db.SaveChanges();
        }
        public int AdđoChoi(int idTK,string tenmh,string gia,string sl,HttpPostedFileBase anhbia,HttpPostedFileBase anh1,HttpPostedFileBase anh2,int loai,int nsx,int km,DateTime ngay,string mota)
        {
            ...
            else
            {
                ...
                db.SaveChanges();
                log.ChiTiet = "Đã THÊM Mô Hình có 'MÃ': " + mh.MaSP + "; 'TÊN' Mô Hình: " + mh.TenSP;
                db.Logs.Add(log);
                db.SaveChanges();
                return 3;
            }
